...
def valid_job():
    ...
    for i in range(90):
        ...
        if cls == "ANIMATION":
            seg["animation_family"] = families[ai % len(families)]
            ai += 1
        timeline.append(seg)
    job = {
        "project": "ORBIT/VSA",
...
class LongformGuardTests(unittest.TestCase):
    ...
    def test_adjacent_same_source_is_rejected(self):
        job = valid_job()
        job["timeline"][1]["source_url"] = job["timeline"][0]["source_url"]
        job["timeline"][1]["start"] = 99.0
