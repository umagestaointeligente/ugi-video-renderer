...
# Try TinyURL legacy endpoint first; it is no-auth and stable for low-volume use.
for alias in ['MateriaisUGI','UGIMateriais','LojaUGI']:
    try:
        r=requests.get('https://tinyurl.com/api-create.php',params={'url':TARGET,'alias':alias},timeout=20,headers={'User-Agent':'UGI-Shortlink-Creator/1.0'})
        body=r.text.strip()
        lines += [f'SERVICE=https://tinyurl.com/api-create.php',f'ALIAS_ATTEMPT={alias}',f'ALIAS_HTTP={r.status_code}',f'ALIAS_RESPONSE={body[:300]}']
        OUT.write_text('\n'.join(lines)+'\n',encoding='utf-8')
        if r.status_code==200 and body.startswith('https://tinyurl.com/'):
            created=body
            break
    except Exception as e:
        lines += [f'ALIAS_ERROR=tinyurl:{alias}:{type(e).__name__}:{str(e)[:220]}']
        OUT.write_text('\n'.join(lines)+'\n',encoding='utf-8')

# Secondary fallbacks.
if not created:
    for service in ['https://is.gd/create.php','https://v.gd/create.php']:
        if created: break
        for alias in ['MateriaisUGI','UGIMateriais','Materiais_UGI','LojaUGI']:
            try:
