...
import argparse, asyncio, concurrent.futures, hashlib, json, os, time
...
def prepare_one(batch,index,prepared_root):
 ...
 root=Path(prepared_root)/rid; root.mkdir(parents=True,exist_ok=True)
 ...
 source_dur=duration(source); voice=root/'voice.mp3'; text=' '.join(item['caption_chunks']); cues=asyncio.run(tts_with_real_boundaries(c,text,item['caption_chunks'],voice)); vd=duration(voice)
 ...
 timeline=_visual_timeline(item,cues,story); mt=item['music_track']; music_raw=CACHE/('music-'+hashlib.sha256(mt['url'].encode()).hexdigest()+'.source')
 ...
 for i,s in enumerate(item['scene_plan']):
  ...
  if source_start+target>source_dur+0.08: raise RuntimeError(f'SCENE_SOURCE_RANGE_FAIL scene={i} start={source_start:.3f} target={target:.3f} source={source_dur:.3f}')
