...
def prepare_one(batch,index,prepared_root):
 ...
 # Fail cheap before downloading large source media: synthesize/validate the exact narration first.
 ...
 # The story ends from the last real spoken boundary, not from trailing encoded silence.
 story=float(cues[-1]['end'])+0.12
 target_min,target_max=(float(x) for x in c['story']['default_total_seconds'])
 if not (target_min<=story<=target_max) and item.get('duration_exception_approved') is not True:
  raise RuntimeError(f'STORY_DURATION_TARGET_FAIL actual={story:.3f} target={target_min:.1f}..{target_max:.1f}')
 ...
 # CTA is also validated before large source/media preparation; identical bad CTA TTS must never waste scene work.
