...
Deno.serve(async(req)=>{
  ...
  if(!auth?.startsWith("Bearer "))return json(401,{error:"AUTH_REQUIRED"});
  ...
  let refresh=false;
  if(req.method==="POST"){
    try{const body=await req.json();refresh=body?.refresh===true}catch{}
  }
  ...
  const {data:plan,error}=await service.rpc(rpc,args);
  if(error)return json(503,{error:"ROLE_SEARCH_PLAN_FAILED"});

  const titles=Array.isArray(plan?.search_titles)?plan.search_titles:[];
  const tiers=titles.reduce((acc:any,item:any)=>{
    const key=String(item?.tier||"other");
    acc[key]=(acc[key]||0)+1;
    return acc;
  },{});
  ...
  return json(200,{
    ...
    summary:{
      ...
      expansion_mode:plan?.expansion_mode||"balanced",
      titles:titles.length,
      scope_terms:Array.isArray(plan?.scope_terms)?plan.scope_terms.length:0,
      tiers,
      matching_champion:"v2.0",
      role_graph_challenger:"v3.0-challenger",
    },
  });
});
