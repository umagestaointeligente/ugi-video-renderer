...
function actionFor(p) {
  if (p.id === 'asaas_direct') {
    return probe.asaas_checkout_e2e === true
      ? 'PROMOTE_LIVE_AND_ROUTE_GREENFIELD_PRODUCTS'
  ...
  if (p.id === 'kiwify_marketplace') return 'SEEK_AUTOMATABLE_PUBLICATION_ROUTE_WITHOUT_TOUCHING_PAYOUT_DESTINATION';
...
const autonomousCandidates = ranked.filter(r => !['WAIT_FOR_USER_CONTROLLED_PAYTO_ADDRESS','KEEP_ADAPTER_READY_AND_STOP_ONLY_AT_LIVE_ACCOUNT_ACTIVATION','PREPARE_ACTOR_AND_STOP_ONLY_AT_KYC','PREPARE_PROVIDER_PACKAGE_AND_STOP_ONLY_AT_PAYOUT_SETUP','VERIFY_PAYOUT_READINESS_BEFORE_CLASSIFYING_RECEIVABLE'].includes(r.next_action));
...
const report = {
  ...
  policy: state.policy,
  asaas_checkout_e2e: probe.asaas_checkout_e2e === true,
