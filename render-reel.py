#!/usr/bin/env python3
"""
UGI Reel Renderer R43.8.2 — DYNAMIC SCENES + MULTI-PROVIDER COMPAT
==================================================================
Objetivo:
- preservar Pexels/Pixabay acquisition no workflow + Kokoro + FFmpeg + GitHub + R2;
- consumir exclusivamente scenes dinâmicas recebidas em VIDEO_SCENES_JSON;
- operar em fail-closed quando as scenes não existirem ou forem inválidas;
...
- registrar metadados experimentais, storyboard e QA por plataforma;
- impedir reentrada de conteúdo legado hardcoded no Trial 03.
...
Mídia esperada do workflow:
  output/media/scene-1.mp4 ... scene-N.mp4
"""
...
MUSIC_ENABLED = (os.getenv("VIDEO_MUSIC_ENABLED") or "true").strip().lower() not in {
    "0", "false", "no", "off"
}
MUSIC_STYLE = (
    os.getenv("VIDEO_MUSIC_STYLE")
    or "business_tech_contemporary_instrumental"
).strip()
...
MUSIC_FALLBACK_MODE = (
    os.getenv("VIDEO_MUSIC_FALLBACK_MODE") or "synthetic"
).strip().lower()
...
BRAND_LOGO = Path(
    (
        os.getenv("VIDEO_BRAND_LOGO")
        or "assets/branding/ugi-symbol-transparent.png"
    ).strip()
)
BRAND_TEXT = (
    os.getenv("VIDEO_BRAND_TEXT")
    or "UGI - UMA GESTÃO INTELIGENTE"
).strip()
...
SMOKE_TEST = (
    os.getenv("VIDEO_SMOKE_TEST") or "false"
).strip().lower() in {"1", "true", "yes", "on"}
SMOKE_TEST_DURATION = float(os.getenv("VIDEO_SMOKE_TEST_DURATION") or "4.0")
SMOKE_TEST_PLATFORM = (
    os.getenv("VIDEO_SMOKE_TEST_PLATFORM") or "instagram"
).strip().lower()
...
# ============================================================
# R43.8.2 — DYNAMIC SCENES / FAIL-CLOSED
# ============================================================
...
def _require_scene_text(value, field: str) -> str:
    ...
    if not value:
        raise RuntimeError(
            f"R43.8.2 campo obrigatório ausente: {field}"
        )
    return value
...
def _platform_text_map(value, field: str, index: int) -> dict:
    if not isinstance(value, dict):
        raise RuntimeError(
            f"R43.8.2 {field} da cena {index} precisa ser objeto."
        )
...
def load_dynamic_scenes() -> list[SceneTemplate]:
    if not VIDEO_SCENES_JSON:
        raise RuntimeError(
            "R43.8.2 FAIL-CLOSED: VIDEO_SCENES_JSON é obrigatório. "
    ...
    try:
        ...
        raise RuntimeError(
            f"R43.8.2 FAIL-CLOSED: VIDEO_SCENES_JSON inválido: {exc}"
        ) from exc
    ...
    if not isinstance(scenes_raw, list):
        raise RuntimeError(
            "R43.8.2 FAIL-CLOSED: payload não contém array scenes."
        )

    if not 4 <= len(scenes_raw) <= 10:
        raise RuntimeError(
            f"R43.8.2 quantidade de cenas inválida: {len(scenes_raw)}."
        )
    ...
    for index, item in enumerate(scenes_raw, start=1):
        if not isinstance(item, dict):
            raise RuntimeError(
                f"R43.8.2 cena {index} precisa ser objeto."
            )
        ...
        if role not in allowed_roles:
            raise RuntimeError(
                f"R43.8.2 role inválido na cena {index}: {role}"
            )
    ...
    if len(scenes) == 6:
        ...
        if actual != expected:
            raise RuntimeError(
                "R43.8.2 ordem narrativa inválida: "
                + ">".join(actual)
            )
...
def synthesize_scene_voice(
    ...
    try:
        ...
        raise RuntimeError(
            "Kokoro TTS não disponível no workflow."
        ) from exc
    ...
    if not chunks:
        raise RuntimeError(
            f"Kokoro não retornou áudio: {platform} cena {index}."
        )
...
def allocate_semantic_timeline(
    platform: str,
    script: list[dict],
    ...
    2) se o total ultrapassa a meta, o sistema reduz respiros;
    3) se sobra tempo, distribui respiro por função narrativa.
    """
    target = (
        SMOKE_TEST_DURATION
        if SMOKE_TEST
        else float(PLATFORM_PROFILES[platform]["target_duration"])
    )
...
def drawtext(
    ...
    return (
        "drawtext="
        f"fontfile='{fontfile}':"
        f"textfile='{esc(textfile)}':reload=0:"
        f"fontsize={size}:"
        f"fontcolor={color}:"
...
def render_scene(
    ...
    visual += [
        ...
        (
            f"drawbox=x=68:y={panel_y}:w=944:h={panel_h}:"
            f"color={PANEL}@0.62:t=fill"
        ),
        (
            f"drawbox=x=68:y={panel_y}:w=8:h={panel_h}:"
            f"color={ACCENT}@0.96:t=fill"
        ),
        drawtext(
            overlay,
            FONT_BOLD,
            ...
            "106",
            str(overlay_y),
            "if(lt(t,0.08),0,min((t-0.08)/0.22,1))",
            12,
        ),
        drawtext(
            support,
            ...
            "106",
            str(support_y),
            "if(lt(t,0.35),0,min((t-0.35)/0.25,1))",
            10,
        ),
    ]
    ...
    if item["role"] == "cta":
        ...
        visual += [
            (
                f"drawbox=x=106:y={cta_y}:w=430:h=82:"
                f"color={ACCENT}@0.94:t=fill"
            ),
            drawtext(
                cta_file,
                FONT_BOLD,
    ...
    visual += [
        (
            f"drawbox=x=82:y={progress_y}:w=916:h=4:"
            "color=white@0.16:t=fill"
        ),
        (
            f"drawbox=x=82:y={progress_y}:"
            f"w='916*min(t/{duration},1)':h=4:"
            f"color={ACCENT}@0.96:t=fill"
        ),
        "vignette=PI/7",
    ...
    if logo_available:
        ...
        filter_complex = (
            ...
            f"color=c=black@0.0:s={lockup_w}x{lockup_h}:"
            f"d={duration},format=rgba[brandbg];"

            f"[1:v]scale={BRAND_LOGO_WIDTH}:-1:flags=lanczos,"
            "format=rgba,"
            "lutrgb=r='0':g='0':b='0',"
            f"colorchannelmixer=aa={BRAND_SHADOW_ALPHA:.3f}"
            "[symbolshadow];"

            "[brandbg][symbolshadow]overlay="
            f"x={BRAND_SHADOW_X}:"
            f"y=(H-h)/2+{BRAND_SHADOW_Y}:"
            "format=auto[brandshadow1];"
            ...
            "[brandshadow1][symbol]overlay="
            "x=0:y=(H-h)/2:format=auto[brand1];"

            "[brand1]drawtext="
            f"fontfile='{FONT_BOLD}':"
            f"textfile='{esc(brand)}':reload=0:"
            f"fontsize=24:"
            f"fontcolor=black@{BRAND_SHADOW_ALPHA:.3f}:"
            f"x={text_x + BRAND_SHADOW_X}:"
            f"y={text_y + BRAND_SHADOW_Y}"
            "[brand2];"
            ...
            f"textfile='{esc(brand)}':reload=0:"
            f"fontsize=24:"
            f"fontcolor=white@{BRAND_OPACITY:.3f}:"
            f"x={text_x}:y={text_y}"
            "[brand3];"
        ...
        print(
            "R43.8.2 BRAND WARNING: símbolo transparente não encontrado em "
...
def select_music_track() -> dict:
    ...
    if candidates:
        ...
        repeat_reason = (
            ...
            if non_recent
                ...
                "all family tracks were recent; "
                "deterministic reuse required"
            )
        )
        ...
        _MUSIC_SELECTION_CACHE = {
            ...
            "reason": (
                ...
                f"{repeat_reason}; "
                "deterministic content-based rotation"
            ),
    ...
    _MUSIC_SELECTION_CACHE = {
        ...
        "source": (
            "synthetic"
            if MUSIC_FALLBACK_MODE == "synthetic"
            else "disabled"
        ),
...
def finalize_platform(
    ...
    graph = (
        ...
        f"afade=t=out:st={max(0.1, duration - 0.8)}:d=0.8"
        "[music];"
        "[music][vsc]sidechaincompress="
        "threshold=0.040:ratio=2.8:attack=18:release=420:makeup=1.10"
        "[ducked];"
        "[ducked][vmix]amix="
    ...
    return {
        ...
        "music_selection_reason": selection.get("reason"),
        "music_recent_tracks_filtered": selection.get(
            "recent_tracks_filtered",
            [],
        ),
        "music_library_candidates": selection.get(
            "library_candidates",
            [],
        ),
...
def validate_video(
    ...
    if abs(actual - expected) > 0.30:
        raise RuntimeError(
            f"Duração fora da tolerância em {path.name}: "
...
def qa_platform(
    ...
    human_scenes = sum(
        1
        for x in media_info
        if x["media_kind"] in {"video", "image"}
    )
    ...
    if SMOKE_TEST:
        checks = {
            ...
            "short_overlay": all(
                len(x["overlay"].split()) <= 9
                for x in script
            ),
            "dynamic_layout": True,
            "safe_zone_applied": True,
            "voice_driven_timeline": True,
            "duration_sum_matches": (
                abs(sum(durations) - SMOKE_TEST_DURATION)
                <= 0.01
            ),
        }
        ...
        checks = {
            ...
            "cta_present": (
                bool(script[-1]["overlay"])
                and bool(
                    PLATFORM_PROFILES[platform]["primary_cta"]
                )
            ),
            "short_overlays": all(
                len(x["overlay"].split()) <= 9
            ...
            "duration_sum_matches": (
                abs(
                    sum(durations)
                    - PLATFORM_PROFILES[platform]["target_duration"]
                )
                <= 0.01
            ),
        }
...
def render_platform(platform: str) -> dict:
    ...
    target = (
        SMOKE_TEST_DURATION
        if SMOKE_TEST
        else float(profile["target_duration"])
    )
    ...
    for item in script:
        ...
        voices.append(voice)
        voice_durations.append(
            ffprobe_duration(voice)
        )

    scene_durations = allocate_semantic_timeline(
        platform,
        script,
        voice_durations,
    )
    ...
    for item, raw_voice, duration in zip(
        script,
        voices,
        scene_durations,
    ):
        ...
        item["voice_source_duration"] = round(
            ffprobe_duration(raw_voice),
            3,
        )
        item["voice_fitted_duration"] = round(
            ffprobe_duration(fitted),
            3,
        )
        item.update(info)
    ...
    voice_timeline = create_voice_timeline(
        platform,
        fitted_voices,
    )
    ...
    storyboard = {
        ...
        "scene_source": "dynamic",
        "scene_count": len(SCENES),
        "scene_roles": [
            scene.role
            for scene in SCENES
        ],
        "legacy_hardcoded_scenes_disabled": True,
        "story_structure": ">".join(
            scene.role
            for scene in SCENES
        ),
        "scenes": script,
    ...
    (
        OUTPUT_DIR
        / f"r42-storyboard-{platform}.json"
    ).write_text(
        json.dumps(
            storyboard,
            ensure_ascii=False,
    ...
    if qa["quality_status"] != "pass":
        raise RuntimeError(
            f"R43.8.2 QA reprovado para {platform}: "
            f"{qa['checks']}"
        )
...
def main() -> int:
    for binary in ("ffmpeg", "ffprobe"):
        if shutil.which(binary) is None:
            raise RuntimeError(
                f"{binary} não encontrado."
            )

    for font in (FONT_REGULAR, FONT_BOLD):
        if not Path(font).exists():
            raise RuntimeError(
                f"Fonte ausente: {font}"
            )
    ...
    print("LEGACY_HARDCODED_SCENES_DISABLED=true")
    print(
    ...
    if SMOKE_TEST:
        platform = (
            SMOKE_TEST_PLATFORM
            if SMOKE_TEST_PLATFORM in PLATFORM_PROFILES
            else "instagram"
        )
    ...
    manifest = {
        "version": "R43_8_2_DYNAMIC_SCENES_MULTI_PROVIDER_COMPAT",
        "render_id": RENDER_ID,
        "title": TITLE,
        "content_id": CONTENT_ID,
        "experiment_id": EXPERIMENT_ID,
        ...
        "smoke_test": {
            ...
            "render_strategy": (
                "single_short_render_plus_compatibility_copies"
                if SMOKE_TEST
                else "full_multi_platform"
            ),
        },
        "brand_signature": {
            "enabled": True,
            ...
            "symbol_render": (
                "white_monochrome_alpha_preserved"
            ),
            "composition": (
                "single_rgba_lockup_layer_with_subtle_shadow"
            ),
            ...
            "fallback": (
                "text_only_if_logo_asset_missing"
            ),
        },
        "music_policy": {
            ...
            "recent_tracks_input": sorted(
                _parse_recent_tracks()
            ),
            "fallback_mode": MUSIC_FALLBACK_MODE,
            "ducking": True,
            "commercial_safety": (
                "use proprietary or royalty-free "
                "instrumental music only"
            ),
            "production_rule": (
                "production selects from "
                "assets/music/<family>; "
                "synthetic fallback is diagnostic only"
            ),
            "anti_repeat": (
                "VIDEO_MUSIC_RECENT_TRACKS excludes "
                "recent filenames when alternatives exist"
            ),
            "rotation": (
                "deterministic SHA-256 rotation by "
                "render/content/experiment/variant"
            ),
        },
        "music_selection": {
            key: (
        ...
        "architecture_note": (
            "R43.8.2 consumes only VIDEO_SCENES_JSON dynamic scenes, "
            "fails closed when scenes are missing/invalid, preserves "
            "the approved audiovisual/branding behavior, and is compatible "
            "with the R43.8.2 workflow provider cascade Pexels -> Pixabay."
        ),
    }

    (
        OUTPUT_DIR
        / "r42-platform-manifest.json"
    ).write_text(
        json.dumps(
            manifest,
            ensure_ascii=False,
